            ...
            if(isset($events[$today]) and count($events[$today]))
            {
                ...
                foreach($events[$today] as $event)
                {
                    ...
                    elseif(trim($start_time)) {
                       $events_str .= '<div class="mec-event-time mec-color">';
                    ...
                    if($this->localtime) $events_str .= MEC_kses::full($this->main->module('local-time.type3', array('event' => $event)));
                    $events_str .= '<div class="mec-event-detail"><div class="mec-event-loc-place"><svg xmlns="http://www.w3.org/2000/svg" width="63" height="62" viewBox="0 0 63 62" fill="none">
<path d="M55.125 25.8334C55.125 43.9167 31.5 59.4167 31.5 59.4167C31.5 59.4167 7.875 43.9167 7.875 25.8334C7.875 19.6671 10.3641 13.7534 14.7946 9.39314C19.2251 5.03292 25.2343 2.58337 31.5 2.58337C37.7657 2.58337 43.7748 5.03292 48.2054 9.39314C52.6359 13.7534 55.125 19.6671 55.125 25.8334Z" fill="#ED1C24"/>
<path d="M31.5 33.5834C35.8492 33.5834 39.375 30.1136 39.375 25.8334C39.375 21.5532 35.8492 18.0834 31.5 18.0834C27.1508 18.0834 23.625 21.5532 23.625 25.8334C23.625 30.1136 27.1508 33.5834 31.5 33.5834Z" fill="white" stroke="white" stroke-linecap="round" stroke-linejoin="round"/>
</svg>'.esc_html((isset($location['name']) ? $location['name'] : '')).'</div></div>';
                    $events_str .= MEC_kses::element($this->booking_button($event));
                    $events_str .= MEC_kses::element($this->display_custom_data($event));
                    $events_str .= MEC_kses::element($this->display_cost($event));
            ...
            else
            {
                ...
                if(!$this->display_all)
                {
                    ...
                    $events_str .= '<div class="mec-event-detail">'.esc_html__('No Events', 'mec').'</div>';
                    $events_str .= '</article>';
                    $events_str .= '</div>';
                }
            }
